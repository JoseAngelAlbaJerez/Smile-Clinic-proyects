...
<div class="row">
<div class="col-2 mt-3"><h5 id="paciente-id">Paciente: {{ $pacient_id }}</h5></div>
<div class="col-8"><h1>Odontodiagrama</h1></div>
...
<div id="radio">
    ...
    <input type="radio" id="radio4" name="accion" value="extraccion" /><label for="radio4"> <i
            class="fas fa-teeth-open"></i> Extraccion</label>
...
<div id="canvasesdiv" style="position:relative; width:810px; height:200px">
    <canvas id="myCanvas" width="810" height="200" style="z-index: 1; position:absolute; left:0px; top:0px;"></canvas>
    <canvas id="myCanvas2" width="810" height="200" style="z-index: 2; position:absolute; left:0px; top:0px;"></canvas>
    <canvas id="myCanvas3" width="810" height="200" style="z-index: 3; position:absolute; left:0px; top:0px;"></canvas>
    <canvas id="myCanvas4" width="810" height="200" style="z-index: 4; position:absolute; left:0px; top:0px;"></canvas>
</div>
...
var diente2 = 0;
// 18 - 28 dientes
const customNumbers = [18, 17, 16, 15, 14, 13, 12, 11, 21, 22, 23, 24, 25, 26, 27, 28];
...
const customNumbersSecondRow = [48, 47, 46, 45, 44, 43, 42, 41, 31, 32, 33, 34, 35, 36, 37, 38];
    ...
    for (var i = 0; i < array_local.length; i++) {
        ...
        if (parseInt(item[0], 10) == diente) {
            ...
            if (acc == 1) {
                ...
                console.log(ctx2, item[0], item[1], color)
                color = 'red';
            ...
            //Cada vez que se asigne un color a una seccion me aumente el valor de caries, rest
...
function saveDataToBackend(data) {
    ...
    html2canvas(document.querySelector("#canvasesdiv")).then(canvas => {
        ...
        var dataURL = canvas.toDataURL('image/png'); // Genera el dataURL de la imagen

        // Agregar la imagen a los datos que se enviarán al backend
        data.image = dataURL;

        // Enviar los datos y la imagen al servidor
        axios.post('/saveData', data)
            .then(response => {
                ...
                setTimeout(() => {
                    window.location.href = '/budget/' + {{ $pacient_id }};
...
saveButton.addEventListener('click', function () {
    ...
    // Llamada a la función para guardar los datos y generar la imagen
    saveDataToBackend(dataToSave);
});


function showSuccessAlert() {
        swal.fire("Odontodiagrama guardado correctamente");
    }

    function showErrorAlert() {
        swal.fire("Error al guardar el Odontodiagrama. Por favor, inténtelo de nuevo.");
    }
